# FreightForce â€” top-level Makefile
...
help:
	@echo "FreightForce â€” common targets"
	@echo ""
...
tools-deploy:  ## Deploy every OSS tool registered for FreightForce via ArgoCD
	kubectl apply -f gitops/argocd/applicationsets/all-tools-applicationset.yaml
	argocd appset get freightforce-tools --grpc-web

tools-status:  ## Show sync status of every registered tool
	argocd app list -l "project=freightforce-tools" --grpc-web
