resource "google_kms_key_ring" "freightforce" {
...
resource "google_kms_crypto_key" "documents" {
  name            = "documents"
  key_ring        = google_kms_key_ring.freightforce.id
  rotation_period = "7776000s"
}

resource "google_secret_manager_secret" "pg" {
  secret_id = "freightforce-${var.environment}-postgres"
  replication { auto {} }
}
