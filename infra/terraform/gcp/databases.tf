resource "google_compute_global_address" "pg" {
  name          = "freightforce-${var.environment}-pg-psa"
  purpose       = "VPC_PEERING"
...
resource "google_sql_user" "freightforce" {
  name     = "freightforce"
  instance = google_sql_database_instance.postgres.name
  password = random_password.pg.result
}

resource "google_sql_database" "freightforce" {
  name     = "freightforce"
  instance = google_sql_database_instance.postgres.name
}
...
resource "google_pubsub_topic" "audit" { name = "freightforce-${var.environment}-audit" }
