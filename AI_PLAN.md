# AI_PLAN.md â€” FreightForce (Logistics & Supply-Chain Platform)
...
- Customs documentation is multilingual + scanned + form-heavy â€” only
  doc-AI scales.
...
| 4 | Load matching (carrier â†” shipper) | freight-broker | Embedding match + LightGBM ranker | <500 ms |
...
`freight-architect` â€” researches AI tooling, proposes services, on-boards
...
By service â€” one agent per microservice (~180). Owns README,
...
```
ai-platform/
â”œâ”€â”€ cluster: freight-ai-{aws,gcp,azure}    â† cloud GPU pool
â”œâ”€â”€ cluster: freight-ai-edge               â† yard/port Jetson Orin nodes
â”œâ”€â”€ namespace: freight-ai-control           â† Paperclip
â”œâ”€â”€ namespace: freight-ai-agents            â† OpenClaw runtime
â”œâ”€â”€ namespace: freight-ai-sandbox           â† NemoClaw
â”œâ”€â”€ namespace: freight-ai-models            â† vLLM, Ollama, LiteLLM, Triton
â”œâ”€â”€ namespace: freight-ai-data              â† Qdrant, Weaviate, MinIO, MLflow
â”œâ”€â”€ namespace: freight-ai-obs               â† Langfuse, Phoenix
â””â”€â”€ namespace: freight-ai-pipelines         â† Argo Workflows
```
...
- Per-region (EU, US, IN, ME, SEA) AI data planes â€” Cilium netpol enforced.
...
| Hours-of-Service / FMCSA / EU AETR | AI scheduling cannot violate HoS â€” hard rule constraints |
...
| 5 | Per-service Tier-2 agents (tms â†’ wms â†’ tracking first) |
